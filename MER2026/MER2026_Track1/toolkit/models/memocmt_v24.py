...
class FiLMFusion(nn.Module):
    """Speaker context modulates the listener face by feature-wise affine
    (scale & shift) instead of a 2-token MHA softmax. Fewer params, no
    unstable softmax over a length-2 sequence."""
    def __init__(self, dim):
        super().__init__()
        self.speaker_encoder = nn.Sequential(
            nn.Linear(dim * 2, dim), nn.GELU(), nn.Linear(dim, dim * 2)
        )
        self.ln = nn.LayerNorm(dim)

    def forward(self, listener_feat, sp_a, sp_t):
        sp = torch.cat([sp_a, sp_t], dim=-1)                 # (B, 2H)
        gamma, beta = self.speaker_encoder(sp).chunk(2, dim=-1)
        return self.ln(listener_feat * (1 + gamma) + beta)   # (B, H)


class MemoCMTV24(nn.Module):
    """
    v13 + FiLM fusion (GP1) + modality dropout (GP2).

    Two orthogonal, capacity-reducing / regularization changes over v13:
      - fusion_type='film' (default): replace the 2-token MHA fusion with a
        FiLM block (speaker -> scale & shift on the listener face). Set
        fusion_type='mha' to keep v13's MHA fusion (for GP2-only ablation).
      - modality_dropout p: during training, with prob p drop EXACTLY ONE of
        the two speaker tokens (audio or text) -> the listener must sometimes
        anchor on a single speaker modality; never both zero (so the fusion
        input is never degenerate, unlike the old speaker_drop_p pathology).
    ...
    def __init__(self, args):
        ...
        self.modality_dropout = getattr(args, 'modality_dropout', 0.0) or 0.0
        self.fusion_type      = getattr(args, 'fusion_type',      'film')
        ...
        if self.fusion_type == 'film':
            self.fusion = FiLMFusion(hidden_dim)
        else:  # 'mha' -> v13-style
    ...
    def forward(self, batch):
        ...
        # Modality dropout: drop EXACTLY ONE speaker modality (never both)
        ...
        # Fusion
        if self.fusion_type == 'film':
            features = self.fusion(listener_feat, sp_a, sp_t)
        else:
