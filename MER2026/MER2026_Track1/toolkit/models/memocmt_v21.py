...
class LearnableQueryPooling(nn.Module):
    """Attention-pool a sequence to one static vector via a learnable query.
    Learns to weight informative frames (a salient word/tone) over filler
    (pauses) -- unlike mean-pooling which weights every frame equally. Output
    is a single (B, H) vector, so it keeps v13's static-speaker robustness
    (no per-frame speaker sequence into fusion -> no v14 alignment trap)."""
...
class CLSTransformerEncoder(nn.Module):
    """Linear proj → prepend CLS → sinusoidal PE → TransformerEncoder → CLS out.
    Identical to v13's video encoder."""
...
class MemoCMTV21(nn.Module):
    """
    v13 + attention-pooling for the speaker branch (replaces mean-pooling).

    Only change vs v13: sp_a = a2t.mean(dim=1) / sp_t = t2a.mean(dim=1)
    -> learnable-query attention pooling. Each of a2t and t2a is pooled to a
    single (B, H) token by its own learnable query, so the model focuses on the
    most informative speaker frames (salient word / tone burst) instead of
    averaging blindly. Output stays static (B, H) -> keeps v13's robustness,
    no per-frame speaker sequence into fusion (avoids the v14 alignment trap).

    Video branch (CLS transformer) and fusion are identical to v13.
    """
    ...
    def __init__(self, args):
        ...
        # NEW: attention-pool each speaker stream (own learnable query)
        self.pool_a = LearnableQueryPooling(hidden_dim, num_heads, dropout)
        self.pool_t = LearnableQueryPooling(hidden_dim, num_heads, dropout)
    ...
    def forward(self, batch):
        ...
        # Listener encoding: CLS token from Transformer
        ...
        # NEW: attention-pool instead of mean-pool -> static (B, H) tokens
        sp_a = self.speaker_drop(self.pool_a(a2t))     # (B, H)
        sp_t = self.speaker_drop(self.pool_t(t2a))     # (B, H)
