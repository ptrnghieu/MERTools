...
class MemoCMTV11(nn.Module):
    """
    v3 + auxiliary VIDEO-ONLY classifier (deep supervision on the video branch).

    In co-train, the video branch tends to under-train: in-domain the speaker
    audio/text are strong (they are the target's OWN voice), so the fusion can
    reach low loss without a great video representation -> video becomes a lazy
    passenger. At test the audio/text belong to the speaker (weak), so the model
    must lean on video -- which was under-trained. v11 forces listener_feat
    (pooled CLIP) to be independently emotion-discriminative via an auxiliary CE
    head, strengthening the residual anchor the fusion relies on at test.

    interloss = lambda_vid * CE(video_head(listener_feat), y), train-only.
    Uses plain CLIP video (AU dropped; AU experiments v9/v10 all regressed).
    """
    ...
    def __init__(self, args):
        ...
        self.lambda_vid     = getattr(args, 'lambda_vid',     0.5)
        ...
        # auxiliary video-only classifier (deep supervision on the anchor)
        self.aux_video_head = nn.Linear(hidden_dim, output_dim1)
    ...
    def forward(self, batch):
        ...
        # deep supervision: force listener_feat (video anchor) to be
        # independently emotion-discriminative
        interloss = torch.zeros(1, device=audio.device).squeeze()
        if self.training and 'emos' in batch:
            aux_logits = self.aux_video_head(listener_feat)
            interloss = self.lambda_vid * F.cross_entropy(aux_logits, batch['emos'])
