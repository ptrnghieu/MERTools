...
class MemoCMTFusion(nn.Module):
    """
    Speaker branch : LSTM → bidir cross-attention (audio↔text) → mean pool → speaker_feat
    Listener branch: LSTM → LearnableQueryPooling → listener_feat
    Fusion         : Q=listener, K/V=speaker cross-attention + residual → classifier

    Cross-pair augmentation (training only):
        With prob cross_pair_p, randomly permute speaker_feat across the batch.
        Listener feat and labels stay fixed → simulates test condition where
        speaker and listener are different people (interlocutor setting).
    """
    ...
    def __init__(self, args):
        ...
        # ── Fusion: Q=listener (B,1,H), K/V=speaker (B,1,H) ───────────────
    ...
    def forward(self, batch):
        ...
        # ── Cross-pair speaker augmentation (training only) ────────────────
        # Randomly permute speaker_feat across batch items.
        # Listener feat and labels are unchanged → directly simulates test
        # condition where speaker and listener are different people.
        if self.training and torch.rand(1).item() < self.cross_pair_p:
            perm = torch.randperm(speaker_feat.size(0), device=audio.device)
            speaker_feat = speaker_feat[perm]

        # ── Modality dropout: zero entire speaker branch ───────────────────
        ...
        # ── Fusion: Q=listener attends K/V=speaker ────────────────────────
