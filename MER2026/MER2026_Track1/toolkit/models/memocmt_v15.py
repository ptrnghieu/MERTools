...
class VideoTransformerEncoder(nn.Module):
    """Linear proj → CLS → sinusoidal PE → TransformerEncoder.
    Returns frame sequence only (CLS discarded — MBT uses the full seq as K/V)."""
...
class MemoCMTV15(nn.Module):
    """
    Multimodal Bottleneck Tokens (MBT) fusion.

    N learnable bottleneck tokens act as the sole communication channel
    between modalities — modalities never attend to each other directly,
    preventing the model from learning spurious train-set alignments
    (speaker audio/text/video are the same person in train; only video
    belongs to the listener at test time).

    Step 1: Bottleneck (Q) queries speaker context F_s1 = concat(F_a, F_t)
    Step 2: Updated bottleneck (Q) queries listener video frames F_v (K/V)
    Step 3: Mean-pool N tokens → classify
    """
    ...
    def __init__(self, args):
        ...
        n_bottleneck      = getattr(args, 'n_bottleneck', 4)
        ...
        # Bottleneck tokens: N learnable vectors, shared across batch
    ...
    def forward(self, batch):
        ...
        # Step 3: Mean-pool N bottleneck tokens → classify
        features = bn_final.mean(dim=1)              # (B, H)
