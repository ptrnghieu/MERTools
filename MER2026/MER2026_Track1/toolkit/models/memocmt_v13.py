...
class CLSTransformerEncoder(nn.Module):
    """
    Linear proj → prepend learnable CLS → sinusoidal PE → TransformerEncoder → CLS output.
    """
    ...
    def forward(self, x):
        ...
        return self.drop_out(x[:, 0, :])                    # (B, H) — CLS token


class MemoCMTV13(nn.Module):
    """
    v3 + Transformer Encoder + CLS token for the listener (video) branch.

    Changes vs v3:
      - Video branch: LSTMSeqEncoder + LearnableQueryPooling
        → CLSTransformerEncoder (Linear proj + CLS + SinusoidalPE + TF Encoder)
      - Speaker branches (audio, text): identical to v3 (LSTM + bidir cross-attn)
      - Fusion: identical to v3 (listener query over 2 speaker tokens)

    New yaml params (with defaults):
      tf_layers: 2   number of TransformerEncoder layers
      tf_heads:  4   number of attention heads (must divide hidden_dim)
    """
    ...
    def __init__(self, args):
        ...
        num_heads = max(1, hidden_dim // 64)  # for speaker cross-attn / fusion

        # Speaker branches (same as v3)
        ...
        # Listener branch — Transformer + CLS (replaces LSTM + LearnableQueryPooling)
        ...
        # Speaker bidir cross-attention (same as v3)
    ...
    def forward(self, batch):
        ...
        # Fusion: listener query attends 2 speaker tokens (same as v3)
