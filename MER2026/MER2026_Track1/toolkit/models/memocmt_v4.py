...
class MemoCMTV4(nn.Module):
    """
    v3 fusion upgraded: instead of mean-pooling each speaker modality to one
    vector, the listener query attends over the FULL frame-level sequences
    a2t and t2a SEPARATELY, then the two results are combined 1:1.

      attn_a = CrossAttn(Q=listener, K/V=a2t)   # gleans over audio frames
      attn_t = CrossAttn(Q=listener, K/V=t2a)   # gleans over text frames
      fused  = LN( 0.5*attn_a + 0.5*attn_t + listener_feat )
    ...
    def forward(self, batch):
        ...
        # balanced 1:1 combine + residual visual
        features = self.fuse_drop(self.norm_fusion(0.5 * attn_a + 0.5 * attn_t + listener_feat))
